...
6. `create_sessions` with `athleteIds` (group training = all attendees) and the blocks. Omit `planId`; it attaches to the active plan. `title` is the badge line on the athlete's day header: short, `·`-separated, e.g. `Priority kata · S&C warm-up`.
...
Sessions are assumed done. Read `athleteNotes` on each session first: the athlete logs skips, actual reps and notes from the portal, so most of what happened is already there. Ask the coach only what is still unclear. `skip_session` for a missed training; `update_block` with `skipped` or `actualReps` for partial work. Then `get_weekly_vli` and report actual vs target.
